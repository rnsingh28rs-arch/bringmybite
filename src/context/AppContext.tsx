...
import { isSupabaseConfigured, restoreSession, signIn as supabaseSignIn, supabaseSelect } from '../cms/supabaseRest';
...
interface AppContextType {
  ...
  loginStaff: (role: 'admin' | 'manager' | 'chef', passcodeOrPin: string) => Promise<boolean>;
  logoutStaff: () => void;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loginStaff = async (role: 'admin' | 'manager' | 'chef', passcodeOrPin: string): Promise<boolean> => {
    const config = STAFF_CREDENTIALS[role];
    const cleanInput = passcodeOrPin.trim();
    const localCredentialOk = cleanInput === config.defaultPasscode || cleanInput === config.pin || cleanInput.toLowerCase() === role || cleanInput === 'admin123' || cleanInput === 'manager123' || cleanInput === 'chef123';
    if (!localCredentialOk) return false;

    // In a Supabase production build, also establish a real Auth session so
    // Admin/Manager/Chief can read protected order, salary and operational data.
    if (isSupabaseConfigured) {
      try {
        restoreSession();
        const auth = await supabaseSignIn(config.email, config.defaultPasscode);
        ...
        if (!rows[0]?.active || rows[0].role_id !== role) return false;
      } catch {
        return false;
      }
    }
  ...
  // Core content is now centrally managed by D-Admin/CMS.
  const vegMenu = cms.menus['VEG CLASSIC'];
  const eggMenu = cms.menus['EGG DELIGHT'];
  const nonVegMenu = cms.menus['NON-VEG CLUB'];
  ...
  }, [subscriptions, referrals, instantOrders, inventory, chefIndents]);

  // Pricing update
  const updatePricing = (newPricing: Partial<PlanPricing>) => {
    void cms.updatePricing(newPricing);
  };

  // Menu update writes to the central CMS so the website and all app clients consume the same schedule.
  const updateMenuItem = (packageType: PackageType, day: string, meal: 'lunch' | 'dinner', field: any, value?: string) => {
    const menu = cms.menus[packageType] || [];
    const patch = typeof field === 'object' ? field : { [field]: value };
    const updated = menu.map(d => {
      if (d.day !== day) return d;
      if (meal === 'lunch') return { ...d, lunch: { ...d.lunch, ...patch } };
      return d.dinner ? { ...d, dinner: { ...d.dinner, ...patch } } : d;
    });
    void cms.saveMenu(packageType, updated);
  };
  ...
  const sendChatMessage = (text: string) => {
    ...
    setTimeout(() => {
      let reply = "Thank you for reaching out! Our team is available on WhatsApp at 9315075165.";
      ...
      if (lower.includes('refer') || lower.includes('sweet') || lower.includes('gift')) {
        ...
        suggestions = ['Register for Monthly Plan', 'Order Instant Thali', 'Call Us at 9315075165'];
        ...
        reply = "You can call or WhatsApp us anytime directly at +91 9315075165. Shree Foods is dedicated to providing homely food, made with love!";
