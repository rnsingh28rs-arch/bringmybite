...
export const StaffLoginModal: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rows = await supabaseSelect<AdminAccount>(
        'bmb_admin_users',
      ...
      if (!account?.active || account.role_id !== selectedRole) {
        signOut();
        throw new Error('This account is not active or is not assigned to the selected staff role.');
      }

      setActiveRole(selectedRole);
      window.dispatchEvent(new CustomEvent('bmb:staff-authenticated', { detail: { role: selectedRole } }));
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-[#FAF7F2] rounded-3xl w-full max-w-lg shadow-2xl border-2 border-[#124E33] overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-200 grid grid-cols-3 gap-2">
          {(['admin', 'manager', 'chef'] as StaffRole[]).map(role => <button key={role} type="button" onClick={() => chooseRole(role)} className={`p-2.5 rounded-xl border text-xs font-bold flex flex-col items-center gap-1.5 ${selectedRole === role ? 'bg-[#124E33] text-white border-[#124E33]' : 'bg-white text-gray-700 border-gray-300'}`}>{roleMeta[role].icon}<span>{role === 'admin' ? 'Admin' : role === 'manager' ? 'Manager' : 'Chef'}</span></button>)}
        </div>
