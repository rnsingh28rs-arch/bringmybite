...
import { ShieldAlert, Briefcase, ChefHat, Lock, X, CheckCircle2, AlertCircle, ShieldCheck, ArrowRight, Sparkles } from 'lucide-react';
...
export const StaffLoginModal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setErrorMsg(''); setSuccessMsg(''); setPin('');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (session.role !== selectedRole) throw new Error('This account is not authorized for this panel.');
      const effectiveRole = selectedRole as 'd_admin'|'admin'|'manager'|'chef';
      setActiveRole(effectiveRole);
      setSuccessMsg('Login successful.');
      window.dispatchEvent(new CustomEvent('bmb:staff-authenticated', { detail: { role: effectiveRole, userId: session.id } }));
      setIsStaffLoginOpen(false); setTargetStaffRole(null); setPin('');
  ...
  const roleMeta: Record<StaffRole, { title:string; description:string; icon:React.ReactNode }> = {
    d_admin: { title:'D-Admin', description:'Director control centre and CMS configuration', icon:<Sparkles className="w-5 h-5"/> },
  ...
  return <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4">
    <div className="bg-[#FAF7F2] rounded-3xl w-full max-w-sm shadow-2xl border-2 border-[#124E33] overflow-hidden">
      ...
      <div className="p-4 grid grid-cols-4 gap-2 border-b border-gray-200">
        {(Object.keys(roleMeta) as StaffRole[]).map(role => <button key={role} type="button" onClick={()=>chooseRole(role)} className={`p-2.5 rounded-xl border text-xs font-bold flex flex-col items-center gap-1.5 ${selectedRole===role?'bg-[#124E33] text-white border-[#124E33]':'bg-white text-gray-700 border-gray-300'}`}>{roleMeta[role].icon}<span>{roleMeta[role].title}</span></button>)}
      </div>
