    ...
    <div className="bg-[#05180F] text-white border-b-2 border-amber-400 py-2.5 px-4 sticky top-0 z-40 shadow-lg">
      <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center bg-black/50 p-1 rounded-xl border border-emerald-900/80 gap-1 text-xs">
          ...
          <button onClick={() => window.location.assign('/d-admin')} className="px-3 py-1.5 rounded-lg font-bold text-amber-300 hover:text-white hover:bg-amber-500/20 transition-all flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => { window.history.pushState({}, '', '/'); window.dispatchEvent(new PopStateEvent('popstate')); setActiveRole('customer'); }}
