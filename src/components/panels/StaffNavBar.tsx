...
export const StaffNavBar: React.FC = () => {
  ...
  const go = (url: string, role: 'admin' | 'manager' | 'chef') => {
    setActiveRole(role);
    ...
    <div className="bg-[#05180F] text-white border-b-2 border-amber-400 py-2.5 px-4 sticky top-0 z-40 shadow-lg">
      <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center bg-black/50 p-1 rounded-xl border border-emerald-900/80 gap-1 text-xs">
          <button type="button" onClick={() => go('/admin#115566', 'admin')} className={`px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5 ${activeRole === 'admin' ? 'bg-red-700 text-white' : 'text-gray-300 hover:text-white'}`}><ShieldAlert className="w-3.5 h-3.5" />Admin</button>
          <button type="button" onClick={() => go('/admin#556611', 'manager')} className={`px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5 ${activeRole === 'manager' ? 'bg-blue-700 text-white' : 'text-gray-300 hover:text-white'}`}><Briefcase className="w-3.5 h-3.5" />Manager</button>
          <button type="button" onClick={() => go('/admin#665511', 'chef')} className={`px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5 ${activeRole === 'chef' ? 'bg-amber-700 text-white' : 'text-gray-300 hover:text-white'}`}><ChefHat className="w-3.5 h-3.5" />Chef</button>
          <button type="button" onClick={() => window.location.assign('/dadmin#505555')} className="px-3 py-1.5 rounded-lg font-bold text-amber-300 hover:text-white hover:bg-amber-500/20 flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5" />D-Admin</button>
        </div>
