      ...
      <div className="max-w-[1500px] mx-auto space-y-5">
        <header className="rounded-2xl bg-[#0C3822] text-white p-5 sm:p-6 shadow-lg border-b-4 border-[#C88A24] flex flex-wrap gap-4 justify-between items-center">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            <button onClick={() => { window.history.pushState({}, '', '/'); window.dispatchEvent(new PopStateEvent('popstate')); }} className="px-3 py-2 rounded-xl bg-amber-400 hover:bg-amber-300 text-black text-xs font-black">← Back to Website</button>
...
function AllPanels({ cms }: { cms: ReturnType<typeof useCms> }) {
  return <div className={card}>
    <div className="flex items-center justify-between gap-3"><div><h2 className="text-xl font-black text-[#124E33]">All Application Panels</h2><p className="text-sm text-gray-500 mt-1">Every configured workspace is listed here. Toggle visibility from Roles & Permissions.</p></div><span className="text-xs font-black px-3 py-2 rounded-xl bg-emerald-50 text-emerald-800">{cms.panels.filter(p => p.active).length} active / {cms.panels.length} total</span></div>
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-5">
      <div className="rounded-2xl border-2 border-amber-300 bg-amber-50 p-4"><div className="flex items-center justify-between gap-2"><span className="font-black text-gray-900">D-Admin Designer</span><span className="text-[10px] px-2 py-1 rounded-full font-black bg-amber-200 text-amber-900">CURRENT</span></div><div className="text-[11px] text-gray-600 mt-2">/d-admin</div><button onClick={() => window.location.assign('/d-admin')} className="mt-3 px-3 py-2 rounded-xl bg-[#124E33] text-white text-xs font-black">Open D-Admin</button></div>
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4"><span className="font-black text-gray-900">Staff Salaries</span><div className="text-[11px] text-gray-600 mt-2">Payroll for Admin, Manager, Chef and Helpers</div><div className="mt-3 text-xs font-bold text-emerald-900">Available in Staff Salaries tab</div></div>
      <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4"><span className="font-black text-gray-900">GST / Corporate Billing</span><div className="text-[11px] text-gray-600 mt-2">Print GST or normal office/tiffin bills</div><div className="mt-3 text-xs font-bold text-blue-900">Available in GST Billing tab</div></div>
      {cms.panels.map(panel => <div key={panel.id} className="rounded-2xl border border-gray-200 bg-gray-50 p-4"><div className="flex items-center justify-between gap-2"><span className="font-black text-gray-900">{panel.name}</span><span className={`text-[10px] px-2 py-1 rounded-full font-black ${panel.active ? 'bg-emerald-100 text-emerald-800' : 'bg-gray-200 text-gray-500'}`}>{panel.active ? 'VISIBLE' : 'HIDDEN'}</span></div><div className="text-[11px] text-gray-500 mt-2 font-mono">{panel.id}</div></div>)}
    </div>
  </div>;
}
...
function Banners({ cms, flash }: any) {
  ...
  const save=async()=>{await cms.saveBanner(draft);flash('Banner saved and synced.');};
  ...
  const handleImage=async(file:File|null)=>{if(!file)return;setUploading(true);try{const data=await new Promise<string>((resolve,reject)=>{const fr=new FileReader();fr.onload=()=>resolve(String(fr.result));fr.onerror=()=>reject(fr.error);fr.readAsDataURL(file);});const img=new Image();img.onload=()=>{const max=1200,scale=Math.min(1,max/img.width);const c=document.createElement('canvas');c.width=Math.round(img.width*scale);c.height=Math.round(img.height*scale);const ctx=c.getContext('2d');if(!ctx)throw new Error('Canvas unavailable');ctx.drawImage(img,0,0,c.width,c.height);const out=c.toDataURL('image/jpeg',0.78);setDraft(d=>({...d,image_url:out,image_alt:d.image_alt||file.name.replace(/\.[^.]+$/,'')}));setUploading(false);};img.onerror=()=>{setUploading(false);alert('Could not read this image.');};img.src=data;}catch{setUploading(false);alert('Image upload failed.');}};
  return <div className="space-y-4"><div className={card}><div className="flex flex-wrap gap-2 items-center justify-between"><div><h2 className="text-xl font-black text-[#124E33]">Slider Banner Manager</h2><p className="text-sm text-gray-500 mt-1">Change image, heading, description, price, buttons and order from D-Admin.</p></div><button onClick={()=>{const b={...blank,id:`banner-${Date.now()}`};setDraft(b);setEditing(b.id)}} className="px-3 py-2 rounded-xl bg-amber-500 font-bold text-sm flex gap-2"><Plus className="w-4 h-4"/>New Banner</button></div><div className="flex gap-2 overflow-x-auto mt-4">{cms.banners.map((b:any)=><button key={b.id} onClick={()=>select(b.id)} className={`px-3 py-2 rounded-xl text-xs font-bold whitespace-nowrap ${editing===b.id?'bg-[#124E33] text-white':'bg-gray-100'}`}>{b.sort_order}. {b.title}</button>)}</div></div><div className={card}><div className="grid md:grid-cols-2 gap-4">{field('Tag','tag')}{field('Title','title')}{field('Price / Highlight','highlight_price')}{field('Period','period')}{field('Thali Rate','thali_rate')}{field('Image URL','image_url')}{field('Image Alt','image_alt')}<label className="text-sm font-bold text-gray-700 md:col-span-2">Upload Banner Image<input type="file" accept="image/*" className={input+' mt-1'} onChange={e=>void handleImage(e.target.files?.[0]||null)}/>{uploading&&<span className="text-xs text-amber-700">Optimizing image…</span>}{draft.image_url&&<img src={draft.image_url} alt={draft.image_alt} className="mt-2 h-40 w-full object-cover rounded-xl border"/>}</label><label className="text-sm font-bold text-gray-700">Description<textarea className={input+' min-h-24'} value={draft.description} onChange={e=>setDraft({...draft,description:e.target.value})}/></label><label className="text-sm font-bold text-gray-700">Features (one per line)<textarea className={input+' min-h-24'} value={draft.features.join('\n')} onChange={e=>setDraft({...draft,features:e.target.value.split('\n').filter(Boolean)})}/></label><label className="text-sm font-bold text-gray-700">Dish highlights (one per line)<textarea className={input+' min-h-24'} value={draft.dish_highlights.join('\n')} onChange={e=>setDraft({...draft,dish_highlights:e.target.value.split('\n').filter(Boolean)})}/></label><label className="text-sm font-bold text-gray-700">Order<input type="number" className={input} value={draft.sort_order} onChange={e=>setDraft({...draft,sort_order:Number(e.target.value)})}/></label></div><div className="flex gap-2 mt-5"><button onClick={save} className="px-4 py-2.5 rounded-xl bg-[#124E33] text-white font-bold flex gap-2"><Save className="w-4 h-4"/>Save Banner</button><button onClick={()=>setDraft({...draft,active:!draft.active})} className={`px-4 py-2.5 rounded-xl font-bold ${draft.active?'bg-emerald-100 text-emerald-800':'bg-gray-200 text-gray-600'}`}>{draft.active?'Active':'Hidden'}</button>{editing&&<button onClick={remove} className="px-4 py-2.5 rounded-xl bg-rose-50 text-rose-700 font-bold flex gap-2"><Trash2 className="w-4 h-4"/>Delete</button>}</div></div></div>;
}
