...
import { isSupabaseConfigured, getCurrentUser, signIn, signOut, supabaseRpc } from '../../cms/supabaseRest';
...
export const DAdminGuard: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    let cancelled = false;
    const restore = async () => {
      if (!isSupabaseConfigured) { if (!cancelled) setReady(true); return; }
      try {
  ...
  const login = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rows = await supabaseRpc<AdminAccount>('bmb_get_staff_account');
      const account = rows[0];
      if (!account?.active || !isDAdminRole(account.role_id)) {
        signOut();
        throw new Error('This account is not authorized for D-Admin.');
      }
      setAuthorized(true);
    } catch (err) {
      setAuthorized(false);
  ...
  return <div className="min-h-screen bg-[#F6F3EC] flex items-center justify-center p-5"><form onSubmit={login} className="w-full max-w-md bg-white rounded-3xl border border-gray-200 shadow-xl p-7 space-y-4"><div className="w-12 h-12 rounded-2xl bg-[#0C3822] text-[#F2C94C] flex items-center justify-center"><Lock className="w-6 h-6"/></div><div><div className="text-[10px] uppercase tracking-[0.22em] text-amber-600 font-black">D-ADMIN DESIGNER</div><h1 className="text-2xl font-black text-[#124E33] mt-1">CEO Cum Director Login</h1><p className="text-sm text-gray-500 mt-1">Sign in with the Supabase account assigned to D-Admin.</p></div><input className="w-full rounded-xl border border-gray-200 bg-white px-3 py-3 text-sm outline-none focus:ring-2 focus:ring-emerald-500" type="email" required autoComplete="username" placeholder="CEO email" value={email} onChange={e=>setEmail(e.target.value)}/><input className="w-full rounded-xl border border-gray-200 bg-white px-3 py-3 text-sm outline-none focus:ring-2 focus:ring-emerald-500" type="password" required autoComplete="current-password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)}/>{error&&<div className="text-sm text-rose-700 bg-rose-50 border border-rose-200 p-3 rounded-xl">{error}</div>}<button disabled={busy} className="w-full py-3 rounded-xl bg-[#124E33] disabled:opacity-60 text-white font-black flex items-center justify-center gap-2"><ShieldCheck className="w-4 h-4 text-[#F2C94C]"/>{busy?'Checking…':'Secure Sign in'}</button></form></div>;
};
