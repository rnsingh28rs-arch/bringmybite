...
import { getCurrentUser, signInWithStaffCredentials, signOut, supabaseRpc } from '../../cms/supabaseRest';
...
export const DAdminGuard:React.FC<{children:React.ReactNode}>=({children})=>{
  ...
  const [username,setUsername]=useState('dadmin');
  ...
  const login=async(e:React.FormEvent)=>{
    ...
    try{
      const session=await signInWithStaffCredentials(username.trim().toLowerCase(),pin.trim());
  ...
  return <div className="min-h-screen bg-[#F6F3EC] flex items-center justify-center p-5"><form onSubmit={login} className="w-full max-w-md bg-white rounded-3xl border border-gray-200 shadow-xl p-7 space-y-4"><div className="flex items-center justify-between"><div className="w-12 h-12 rounded-2xl bg-[#0C3822] text-[#F2C94C] flex items-center justify-center"><Lock className="w-6 h-6"/></div><button type="button" aria-label="Close D-Admin login" onClick={close} className="p-2 rounded-xl text-gray-400 hover:text-gray-700"><X className="w-5 h-5"/></button></div><div><div className="text-[10px] uppercase tracking-[0.22em] text-amber-600 font-black">D-ADMIN</div><h1 className="text-2xl font-black text-[#124E33] mt-1">Director Login</h1><p className="text-sm text-gray-500 mt-1">Username + configured 6-digit PIN.</p></div><input className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 outline-none focus:ring-2 focus:ring-emerald-500" type="text" autoCapitalize="none" autoCorrect="off" autoComplete="username" value={username} onChange={e=>setUsername(e.target.value.toLowerCase().replace(/[^a-z0-9._-]/g,'').slice(0,32))} placeholder="dadmin"/><input className="w-full rounded-xl border border-gray-200 bg-white px-4 py-4 text-center text-2xl tracking-[0.45em] font-mono outline-none focus:ring-2 focus:ring-emerald-500" type="password" inputMode="numeric" pattern="[0-9]{6}" minLength={6} maxLength={6} autoComplete="current-password" placeholder="••••••" value={pin} onChange={e=>setPin(e.target.value.replace(/\D/g,'').slice(0,6))}/>{error&&<div className="text-sm text-rose-700 bg-rose-50 border border-rose-200 p-3 rounded-xl">{error}</div>}<button disabled={busy} className="w-full py-3 rounded-xl bg-[#124E33] disabled:opacity-60 text-white font-black flex items-center justify-center gap-2"><ShieldCheck className="w-4 h-4 text-[#F2C94C]"/>{busy?'Checking…':'Enter D-Admin'}</button></form></div>;
};
