...
export const InstantOrderModal: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!customerName.trim() || !mobileNumber.trim() || !locationDetail.trim()) { alert('Please fill in your name, contact number, and delivery gate location.'); return; }
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5 bg-[#FAF7F2]">
          {placedOrder ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end"><button type="submit" className="px-6 py-3 rounded-xl bg-[#124E33] text-white font-bold">Submit Instant Order • {configuredRateText || 'Price not set'}</button></div>
            </form>
          )}
        </div>
      </div>
